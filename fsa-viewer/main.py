...
# --------------------------------------------------
_input_csv_dict = {}
with open('A1_25-417-043_1-10_Sample_20210806_130052.fsa.csv') as input_csv:
    for line in input_csv.readlines()[1:]:
        line_info = [thing.replace('"', '') for thing in line.strip().split(',')]
        dye = line_info[0].strip()
        if dye not in _input_csv_dict: _input_csv_dict[dye] = {
                'pos': [],
                'heights': [],
        }
        if not line_info[2].replace('"', '').strip(): continue
        pos = float(line_info[2].replace('"', ''))
        height = int(float(line_info[3].replace('"', '')))
        _input_csv_dict[dye]['pos'].append(pos)
        _input_csv_dict[dye]['heights'].append(height)



    fig = go.Figure()
    for colour in _input_csv_dict:
        hoverval = 'skip' if colour.lower()=='orange' else 'text'
        fig.add_trace(
            go.Bar(
                x=_input_csv_dict[colour]['pos'],
                y=_input_csv_dict[colour]['heights'],
                #hoverinfo=hoverval,
                marker=dict(
                    color=colour,
                )
