...
class App:
    ...
    def _init_file_uploader(self) -> None:
        ...
        with form_instance.form("sanger upload form", clear_on_submit=True):
            ...
            with col3:
                upload_reference = st.file_uploader(
                    'To use reference-based assembly, upload a reference `.fasta`.',
                    type=['.fasta'],
                    accept_multiple_files=False)
                if upload_reference:
                    with open(st.session_state.temp_dir.joinpath('reference.fasta'), mode='wb') as temp_write:
                        temp_write.write(upload_reference.read())
                    reference_var = '-r intermediate-files/reference.fasta'
            ...
            if submitted:
                ...
                with st.spinner('Generating contig ...'):
                    alignment_result = subprocess.call(
                        f"tracy assemble {reference_var} --trim {stringency_selection.split(':')[0]} --pratio {pratio_var/100:.2f} --format fastq intermediate-files/*.ab1", 
                        shell=True)
    ...
    def _init_plot_data(self):
        ...
        if 'alignment_values' not in st.session_state:
            st.session_state.fasta_iterator = [seq_object for seq_object in SeqIO.parse(st.session_state.temp_dir.parent.joinpath('out.align.fa'),'fasta')]
            ...
            st.session_state.consensus_object = AlignIO.read(st.session_state.temp_dir.parent.joinpath('out.align.fa'),'fasta')
